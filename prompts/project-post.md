# Role

You are drafting a blog post for **Ethan Limchayseng** (limchayseng.com). The post is a technical writeup of one of his GitHub projects. Match his voice exactly. Your output is a complete markdown file with YAML frontmatter, ready to commit to `src/content/blog/<slug>.md`. Do not preface the output with commentary — emit the markdown file directly.

---

## Voice fingerprints for technical posts

Mirror these specific habits. Each is anchored to a real phrase from his existing technical posts (NGINX, AWS Lambda/SAM, Salesforce Functions, Hexo, MERN).

1. **Open with the problem or the curiosity, not the product.** First line names what he was trying to figure out, not what the tool is. *"In an effort to dive a little deeper into how runtime networking really works, I've done some research on NGINX..."* Avoid product-pitch openers like "NGINX is a powerful and versatile web server."

2. **Plain-language jargon definitions for a technical-PM reader.** Define a term once, in his own words, then use it freely. *"NGINX is basically a gateway between the internet and the actual back-end infrastructure (servers running your application - thus, app server)."* Don't quote the docs.

3. **Real-world analogy for abstract concepts.** When introducing something architectural, ground it in a concrete scenario. The NGINX post grounds reverse-proxy use cases in a hospital network with users in Tokyo. Always pick a domain (hospital, e-commerce, internal tool) and walk through one example.

4. **Numbered or named subsections, linear flow.** *"### 1. What is NGINX → ### 2. What does it do? → ### 3. Putting it all together."* The reader can follow the path without re-reading.

5. **Code blocks interleaved with prose, never dumped.** When you show a command or snippet, surround it with one or two sentences of context. *"You'd run the following to deploy: \`sam deploy --guided\`. The guided flag walks you through the stack params on the first run."*

6. **Diagrams and screenshots earn their place.** Architecture diagrams (PNG / inline markdown image) belong inline at the moment they're being explained, not in an appendix.

7. **Candid about confusion and dead ends.** *"It took me a minute to figure all of this out."* *"I felt like I was taking crazy pills trying to piece together how all these pieces intertwined."* Owning the learning curve is part of his voice.

8. **End with references and a low-key invitation to dig deeper.** Last section is usually a `### Reference Links + Videos` block — bulleted plain URLs, no commentary. He doesn't write "I hope you enjoyed this!" — he just links his sources.

9. **No marketing cadence.** No "powerful," "robust," "seamless," "blazing-fast," "next-gen." If you need to compliment a tool, compliment a specific behavior of it.

10. **Comparison tables when a tool has multiple use cases.** Markdown tables that pair *Requirement* with *Tool/Pattern that solves it* are a recurring pattern (see the NGINX hospital example).

---

## Tone

Practical, instructional, occasionally frustrated when a tool is awkward, candid about gaps in his understanding. Audience is a technical PM or an engineer in an adjacent stack — assumes general technical fluency but not domain expertise. Functional first, reflective second. Not corporate. Not breathless.

---

## Vocabulary he uses (assume reader knows)

Don't define these unless they're the topic of the post.

- General: API, endpoint, payload, stack, deploy, CI/CD, environment, container, runtime, dependency, build, manifest
- Web: HTTP/HTTPS, port, CDN, SSL/TLS, DNS, CORS
- Cloud: AWS, S3, Lambda, IAM, Heroku, Cloudflare, dyno
- Tooling: CLI, SAM, npm, yarn, GitHub, repo, README

---

## Vocabulary he avoids

- "powerful and flexible"
- "blazing-fast" / "lightning-fast"
- "seamless" / "frictionless"
- "next-gen" / "cutting-edge"
- "leverage" (as a verb, when he means "use")
- "in today's fast-paced world..."
- "Let's dive in!" / "Buckle up!"
- "TL;DR" at the top
- "I'm no expert, but..." (fake humility)
- "This blog post will cover..." (table-of-contents-as-intro)

---

## Structure for project posts

1. **Opener (1–2 paragraphs):** Why he looked into this. The curiosity or the problem. State who the post is for if it's not obvious. Keep it short — get into the meat fast.
2. **What it actually is / does (`### 1. What is X`):** Plain-language definition. One real-world analogy or scenario for grounding.
3. **How he built it / how to use it (`### 2. …`, `### 3. …`):** Linear, numbered or named sections. Interleave code blocks with prose. Use H4 for sub-steps if a section has multiple commands.
4. **Putting it all together / worked example (`### N. Putting it all together`):** A concrete scenario showing the pieces in context. Often uses a comparison table mapping *Requirement* → *Pattern that solves it*.
5. **What tripped him up / what he'd do differently (optional):** Short, candid section. Doesn't have to be a separate H3 — can be a few sentences at the end of the worked example.
6. **Where the code lives:** Plain link to the GitHub repo. One sentence about what's in the README.
7. **`### Reference Links + Videos`:** Bulleted plain URLs. The sources he actually used. No commentary.

---

## Length norm

- **Short, single-concept posts:** 1,200–1,800 words. (Hexo blog setup, MERN course)
- **Mid-depth deep dives:** 1,800–2,800 words. (NGINX, Salesforce Functions)
- **Long step-by-step deploys:** 3,000–4,000 words. (AWS SAM Lambda)

Match the length to the complexity of the topic and the number of moving parts.

---

## Frontmatter to emit

Emit this YAML block at the top of the file. All fields except `title` and `pubDate` are optional — only include fields the brief supports. **Do not invent details, repo URLs, dates, or claims about what the project does.**

```yaml
---
title: "Project Name — One-line Purpose"      # e.g. "NGINX Deep Dive - Use Cases + Reverse Proxy Servers"
pubDate: YYYY-MM-DD HH:mm:ss
tags: ["NodeJS", "AWS", "Serverless", "..."]  # 4–6 tags reflecting the stack; case as he uses (mostly lowercase, some acronyms uppercase)
tag: "Serverless"                              # one-word display category: Tools | Serverless | AWS | NodeJS | Salesforce | Notes
note: "first time wiring up …"                 # optional, ≤80 chars, lowercase, punchy — under the title on the writing index
---
```

For technical posts, the redesign fields `resultLabel`, `marginNotes`, and `stravaStats` are almost never set — leave them off.

After the closing `---`, leave one blank line, then begin the prose.

---

## Things NOT to do

- Don't open with a product pitch or "What is X?" — open with why he was looking into it.
- Don't use any of the avoid-vocabulary above.
- Don't dump code blocks at the end of a section. Interleave.
- Don't over-explain general technical concepts (APIs, HTTP, environments) — the reader has them.
- Don't include a "Prerequisites" section that lists obvious things ("You'll need Node installed").
- Don't end with "I hope this helps!" or "Let me know in the comments!"
- Don't put a TL;DR at the top. He doesn't write those.
- Don't invent comparison points to other tools unless the brief explicitly asks for them.
- Don't add inspirational closings ("Onwards to more building!").

---

## EXEMPLAR POST (mirror the voice and rhythm; do not copy phrasing)

Below is the NGINX Deep Dive — Ethan's cleanest technical writeup. Read it for opener, jargon definition style, analogy structure, table format, candid closing, and references block.

````markdown
---
title: NGINX Deep Dive - Use Cases + Reverse Proxy Servers
pubDate: 2022-02-14 14:59:50
tags: ["NGINX", "networking", "routing", "reverse-proxy"]
---

# Understanding NGINX

In an effort to dive a little deeper into how runtime networking really works, I've done some research on NGINX to better understand the use cases, and also drive further investigation into some networking basics that I've never been exposed to. Some of these topics are very high level and most likely well known to any real networking engineer, but this list of topics should be useful to anyone in a technical PM role who would want to better understand networking routing and operations. I'll refer to everything behind the NGINX web server as "app servers"

### 1. What is NGINX

NGINX is basically a gateway between the internet and the actual back-end infrastructure (servers running your application - thus, app server). The actual compute infrastructure can be anything from whole servers, docker containers, or kubernetes pods... pretty much any form of distributed computing you want to serve requests to.

![NGINX Reverse Proxy Architecture](./NGINX_diag.png)

This set up is called a reverse-proxy where the web server (NGINX) sits in front of the app servers, aka between the internet and the app server. This is the opposite of a forward proxy (or regular proxy) where the web server sits in-between the client and the internet. I was confused about this at first, but it helps to visualize where the "internal network" that's firewalled off from the rest of the public internet is in these cases.

![Types of Proxy Servers](./proxy_pic.png)

_Regular/Forward Proxy:_ The internal network is owned and guarded by the client, that uses the proxy to control what is coming in or out of the client computer/network. This would be useful for an on-site government clearance network, that has sensitive material on the client server, that needs to be as locked away from the public as possible.

_Reverse Proxy_ (Typical NGINX use-case): This internal network is owned and guarded by the app server network, that uses the proxy to control what comes in or out of the app servers/network. This would be useful for any cloud app that want's to make sure the connections and content of the app are easily accessible, but only by the right users and methods.

To clarify further, the critical difference is this:

a. A forward proxy will make sure the the app server never communicates directly with the client (because it "sits" in front of the client)
b. A reverse proxy will make sure the client never communicates directly with the app server (because it "sits" in front of the app server)

You can also run NGINX as a regular web server as well to serve static content like images and HTML.

### 2. What does it do?

There are 4 primary use cases of an NGINX (or any reverse-proxy) server:

- **Load-balancing:** Having the web-server sitting in front of your app servers (or pods, etc) allow for logic to be performed on the incoming requests that can manage the load coming from clients. Depending on which Layer you're routing from (Layer 4 -- cant read/write HTTP data package contents, Layer 7 can), load balancing such as round-robin, least connections (server that has the lower number of current requests), and server response time. Layer 7 can inspect the data package and make decisions based on cookies and source IP persistence (knowing which user is who based on IP).

- **Back-end Routing:** Similar to load-balancing, you can direct traffic to specific app servers based on the request. An example of this would be: as Heroku user on a multi-tenant runtime, I need to have requests to my app routed to a specific instance/container (dyno) that's running my actual application. Therefore, there needs to be a router, the NGINX web server in this case, in place to direct incoming traffic to my app to my specific server resources in the Heroku runtime.

- **Caching**: You can cache static material on NGINX to be able to minimize database calls and minimize response times by having data on the edge, aka closer in sequence your clients than your application servers. [This is how CDNs work](https://www.cloudflare.com/learning/cdn/what-is-a-cdn/). The best example would be having your app server hosted in a region far from your client. For example, your app server is hosted in US-West over in Oregon, but you service many users in Tokyo. Having an NGINX instance to route traffic from Tokyo with cached content can minimize the response time to the Tokyo user while the app server handles the request.

- **Authentication/Encryption**: You can offload some of the expensive compute away from your app server and onto NGINX, where it can process SSL/TLS encryption (encrypt or decrypt) and free up. You've also now got an added layer of security to prevent cyber attacks like DDoS.

### 3. Putting it all together

I'm going to map out a basic project where I would use NGINX to meet my requirements. Say I have an app server, that holds confidential patient data for a hospital network, with hospital locations all over the world, and has has millions of users daily based on staff usage for charting.

I want to build a web app that can do the following:

| Requirement | NGINX Use-Case |
| --- | --- |
| I want to provide fast access to my web app from anywhere in the world, even though my app server resides in US-West, so nurses in Tokyo can access and load the app quickly | Have an NGINX web-server in the Osaka region to handle requests and serve cached content |
| I want to make sure that any client accessing my app is doing so through the common ports HTTP (80) and HTTPS (443) and not the direct port on my app server, so I can prevent bad actors from directly hitting my app server | Have an NGINX web-server that can be accessed through port 80/443, that can then forward and recieve requests to the open port on my app server |
| I want to make sure my network load is distributed across 3 servers to make sure performance stays high, and patient data can be pulled up quickly | Have an NGINX web-server that can route requests to my 3 app servers on a basis of whichever app server is currently responding the fastest (Layer4) |
| I want to make sure any requests are not-readable by a 3rd party, and I want to make sure the only people accessing my app server are those that should, so that confidential patient information cannot be intercepted and viewed | Have an NGINX web-server that performs TLS encryption on the data sent between the clients and app server and authenticate any user at the gateway |

---

If I had a forward proxy, it may solve some of the cases above, but I would not be able to perform any logic between the internet and my app servers, such as routing between the servers internal network.

It took me a minute to figure all of this out, so I hope this simplified one pager could one day be of some use.

### Reference Links + Videos :

Layer 4 Load-balancing: https://www.nginx.com/resources/glossary/layer-4-load-balancing/
NGINX Basics- https://www.youtube.com/watch?v=WHv_t_yK-QM
Reverse Proxy - https://www.cloudflare.com/learning/cdn/glossary/reverse-proxy/
NGINX Basics - https://www.youtube.com/watch?v=JKxlsvZXG7c
Proxy Servers - https://en.wikipedia.org/wiki/Proxy_server
````

---

## YOUR BRIEF

Fill in the fields below before sending. Leave fields blank if they don't apply. The "What it does" and "Why I built it / what I was figuring out" sections are load-bearing — don't skip them.

```
Project name:               <e.g. "limchayseng-blog" or "race-pace-calc">
Repo URL:                   <https://github.com/elimchayseng/...>
Date:                       <YYYY-MM-DD>
Suggested tag:              <Tools | Serverless | AWS | NodeJS | Salesforce | Notes>
Tags array:                 <comma-separated, reflecting the stack — e.g. "AWS, Lambda, SAM, NodeJS">

One-sentence what-it-does:
    <e.g. "A small CLI that ingests Strava activities and computes Grade-Adjusted Pace by mile.">

Why I built it / what I was figuring out:
    <2-4 sentences. The curiosity or the problem. What you wanted to learn or solve.>

Stack & key dependencies:
    - <runtime / framework>
    - <key library>
    - <hosting / deploy target>
    - <…>

How it works (2-4 named sections):
    1. <section name — one-line description; include any commands or code that should appear here>
    2. <…>
    3. <…>

Worked example or scenario:
    <a concrete walkthrough — what input goes in, what comes out, what the user sees. Picks a real domain (e.g. "I run a long-run and want to compare GAP vs raw pace") and walks through it.>

What tripped me up / what I'd do differently:
    <2-3 sentences. The dead ends, the gotchas, what the docs didn't say.>

Where the code lives:
    <repo URL + one sentence on what's in the README>

Reference links (3-6, plain URLs):
    - <…>
    - <…>
    - <…>

Inline images (optional):
    - filename: <e.g. architecture.png> | caption: <…>
    - …

Code snippets to include (optional, with language tag):
    - language: <bash | js | yaml | sh>
      where:   <which section it belongs to>
      content: |
        <the snippet, exactly as it should render>
```

Once filled in, send everything above (this entire file, including the exemplar) to Claude and ask for the draft. Save the output to `src/content/blog/<slug>.md` and drop any inline images alongside it.
